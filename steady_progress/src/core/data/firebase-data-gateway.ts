...
export class FirebaseDataGateway implements DataGateway {
  watchCollection(path: string, query: QuerySpec, onData: (rows: DocumentRow[]) => void, onError: (error: unknown) => void): Unsubscribe {
    return onSnapshot(
      queryFor(path, query),
      (snapshot) => {
        onData(snapshot.docs.map((item) => ({ id: item.id, data: item.data({ serverTimestamps: 'estimate' }) })));
      },
      (error) => {
        console.error('[GATEWAY_WATCH_COLLECTION_ERROR]', path, error);
        onError(error);
      },
    );
  }
  watchDocument(path: string, onData: (row: DocumentRow | null) => void, onError: (error: unknown) => void): Unsubscribe {
    return onSnapshot(
      ...
      (error) => {
        console.error('[GATEWAY_WATCH_DOCUMENT_ERROR]', path, error);
        onError(error);
      },
    );
  }
