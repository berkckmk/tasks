...
class _LogWorkoutSheetState extends ConsumerState<_LogWorkoutSheet> {
      ...
      child: SingleChildScrollView(
        child: Column(
          ...
          children: [
            ...
            ..._exercises.asMap().entries.map((entry) => _exerciseRow(entry.key, entry.value)),
            TextButton.icon(
              onPressed: () => setState(() => _exercises.add(_ExerciseRowData())),
  ...
  Future<void> _save() async {
    ...
    try {
      await ref.read(workoutActionsProvider).logWorkout(name: name, date: _date, exercises: drafts);
      navigator.pop();
    } catch (e) {
      messenger.showSnackBar(SnackBar(content: Text("Couldn't save workout: $e")));
    } finally {
